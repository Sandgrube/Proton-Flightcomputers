...
const int PYRO_BACKUP_PIN = 13;
const float DECELERATION_THRESHOLD = -1.0f;  // Bremsbeschleunigung < -1g (anpassbar)
...
void extendedSetupHooks() {
  ...
  pinMode(PYRO_BACKUP_PIN, OUTPUT);
  digitalWrite(PYRO_BACKUP_PIN, LOW);
...
void deployParachutes() {
  ...
  chuteDeployed = true;
  chuteDeployTime = millis();
}


void checkChuteDeceleration() {
  if (!chuteDeployed || backupTriggered) return;

  unsigned long now = millis();
  if (now - chuteDeployTime < 1500) return;  // Noch nicht genug Zeit vergangen
  ...
  if (accG > DECELERATION_THRESHOLD) {
    Serial.println("⚠️ Keine deutliche Verzögerung erkannt. Backup-Pyro wird aktiviert!");
    digitalWrite(PYRO_BACKUP_PIN, HIGH);
    delay(2000);
    digitalWrite(PYRO_BACKUP_PIN, LOW);
    backupTriggered = true;
  } else {
    Serial.println("✅ Fallschirm scheint zu wirken. Keine Backup-Zündung notwendig.");
    backupTriggered = true; // Einmal prüfen reicht
  }
}
