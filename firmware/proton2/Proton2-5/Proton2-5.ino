...
// ========================= Fallschirm Servo =========================
Servo chuteServo; // global, oben definieren
#define CHUTE_SERVO_PIN 33

// ========================= Pyrokanäle =========================
#define PYRO_1_PIN 13
#define PYRO_2_PIN 12

// ========================= Fallschirm =========================
bool apogeeDetected = false;
bool chuteDeployed = false;
float lastAccelZ = 0.0;
unsigned long apogeeTimerStart = 0;
const float apogeeThreshold = -3.0;   // m/s², ungefähr 0.3g, kann feinjustiert werden
const unsigned long apogeeConfirmTime = 200; // ms, Zeitdauer unter Threshold zur Bestätigung

// Für Fallschirmauslösung Timing und Backup
unsigned long chuteDeployTime = 0;
unsigned long lastBackupTrigger = 0;
int backupStep = 0;
float lastVerticalSpeed = 0.0f;

// Für Beschleunigungswerte (diese sollten auch schon in loop() deklariert sein,
// hier als global, damit parachuteBackup darauf zugreifen kann)
float accX = 0.0f;
float accY = 0.0f;
float accZ = 0.0f;
float accelZ = 0.0f;
...
void deployParachute() {
  chuteServo.write(90);  // z.B. 90° schaltet Fallschirm aus
  Serial.println("[INFO] Fallschirm ausgelöst");
}
...
void setup() {
 ...
 // --- I2C / IMU ---
  ...
  chuteServo.write(0); // Safe Position beim Start
...
//Fallschirm Sinkrate und Backup

void parachuteBackup(float currentVerticalSpeed) {
  if (!chuteDeployed) return;

  unsigned long now = millis();

  if (chuteDeployTime == 0) {
    chuteDeployTime = now;
    lastBackupTrigger = now;
    backupStep = 1;
    lastVerticalSpeed = currentVerticalSpeed;
    return;
  }

  float sinkrateChange = lastVerticalSpeed - currentVerticalSpeed; 
  lastVerticalSpeed = currentVerticalSpeed;

  if ((now - chuteDeployTime > 1000) && (sinkrateChange <= 0)) {
    switch (backupStep) {
      case 1:
...
void loop() {
  ...
  if (!apogeeDetected) {
  if (accelZ < apogeeThreshold) {
    if (apogeeTimerStart == 0) {
      apogeeTimerStart = millis(); // Timer starten
    } else if (millis() - apogeeTimerStart > apogeeConfirmTime) {
      apogeeDetected = true;
      Serial.println("[INFO] Apogäum erkannt!");
    }
  } else {
    apogeeTimerStart = 0; // Reset Timer wenn Beschleunigung wieder über Schwelle
  }
  

  if (apogeeDetected && !chuteDeployed) {
  chuteDeployed = true;
  deployParachute();
  }
  ...
  float accelZ = a.acceleration.z;  // Vertikale Beschleunigung (oder besser: Gesamtabfrage der Beschleunigung)
  float accelTotal = sqrt(accX*accX + accY*accY + accZ*accZ);
