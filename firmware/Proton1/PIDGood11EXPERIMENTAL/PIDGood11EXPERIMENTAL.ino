...
//TESTCODE ACC BEFEHLE

// --- Struktur für empfangene Befehle ---
typedef struct {
  char command[16];  // Befehlsname (z.B. "Liftoff", "Abort")
  float value;       // Optionaler Wert für den Befehl
} CommandData;

CommandData receivedCommand; // Variable für empfangene Befehle

// --- Callback für Empfang von Daten ---
...
void sendeSensordaten() {
  ...
  sensorData.altitude = bmp.readAltitude(101325); // 98400 regensburg | Meereshöhe kalibriert 101325
...
int aCallCount = 0; // für fallschirmservo attach **


void a() {
  parachuteServo.attach(2);
  Serial.println("Funktion a() wurde aufgerufen.");
}
...
//---Sketchy but Working---
//Fallschirm Attach() Zähler logik
  unsigned long currentMillis = millis();

   if (aCallCount < 2) { // Überprüfen, ob a() weniger als zweimal aufgerufen wurde
    a();               // a() aufrufen
    aCallCount++;      // Zähler erhöhen
  }
...
bool detectLiftoff() {
  ...
  if (abs(accelY) > liftoffThreshold && abs(accelY - lastAccelerationY) > 1.0) {  // Änderung der Beschleunigung
    ...
    delay(1000); // 5 Sekunden warten nach Starterkennung
    return true;
  }
...
void deployingChutes() {
  ...
  if (a.acceleration.y < fallThreshold) {
    ...
    sensorData.FallschirmState = 1; // Fallschirm ausgelöst
  } else {
    sensorData.FallschirmState = 0; // Fallschirm nicht ausgelöst
  }

  // Zustand des Fallschirms über ESP-NOW senden
  esp_err_t result = esp_now_send(broadcastAddress, (uint8_t *)&sensorData, sizeof(sensorData));
  Serial.println(result == ESP_OK ? "Fallschirmstatus gesendet" : "Fehler beim Senden des Fallschirmstatus");
}
